...
- [ ] **[CMD-8] Rework ThreatProvider ranking for Era** (2026-06-14, from VERIF-3) — Era scoreboard never populates `damageDone`/`healingDone` (always 0), so the current damage/healing ranking + `healing>1.5×damage` healer inference are dead on Era. New plan: KILL priority by `killingBlows`/`honorableKills`; healer detection by **class prior** (`HEALER_CAPABLE`) and/or **Details!** (VERIF-6). Blocks CMD-6/7 from being meaningful on Era. See `enemy-threat-research.md` In-game findings.
  - _Progress (2026-06-20):_ CLEU aggregate now ranks real damage/healing live (Intel panel: aligned table, Heal column, post-match retention, reload persistence). **Open sub-bug:** `heals>0` alone mis-flags casters as healers (Elemental shaman Ña, warriors/locks self-healing). Fix: heal-on-allies dominance + spec inference — gated on `Research/spec-detection-research.md` ([SPEC-1] spike, requested 2026-06-20).
- [ ] **[SPEC-1] Research: how addons detect enemy spec/role on Classic Era** (2026-06-20) — survey how arena/PvP addons infer spec without the retail spec API: combat-log spell→spec mapping, talent inspection (`NotifyInspect`/`C_SpecializationInfo`), aura/buff signatures, GladiatorlossA2/sArena/spec-detection libraries. Deliverable: `Research/spec-detection-research.md` comparison table + a recommendation for distinguishing Elemental/Enhancement/Resto shaman (and caster vs melee vs healer generally) usable in the CLEU ThreatProvider. Unblocks the CMD-8 healer/spec sub-bug.

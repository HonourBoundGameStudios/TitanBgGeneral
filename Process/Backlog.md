...
- [x] **[CMD-2] Opening split caller** (2026-07-04; **in-game GREEN PENDING**) — an **Openers** section on the Battle Plan board (CMD-1 hub): BG-aware preset buttons (AB "5 Stables, 5 Blacksmith, rest Farm" etc; WSG/AV sets). **Left-click broadcasts** the preset to the raid via `GetChatType()`; **right-click edits** it in a native `StaticPopup` text box. Presets persist per-BG under `TitanBgGeneralSaved.openers[bg]`, seeded from defaults on first open, so edits stick across sessions. **Verify next session:** open `/bgplan` in a BG, click a preset (lands in chat), right-click → edit → Save → confirm the new text persists after `/reload`.
- [x] **[CMD-3] Raid-marker integration** (2026-07-04; **in-game GREEN PENDING**) — a **Markers** section on the Battle Plan board: 8 raid-target icon buttons (star…skull) mark your current target via `SetRaidTarget("target", i)`; **Unmark target** clears it; **Skull EFC** (WSG) resolves the enemy flag carrier's live unit (`FlagState.CarrierUnit("efc")`) and skulls it. `CanMark()` warns when you lack raid lead/assist (marking silently no-ops otherwise). Tooltips name each icon. **Verify next session:** with lead/assist, target an enemy and click icons; in WSG target the EFC and hit Skull EFC.
- [x] **[CMD-4] Custom callout editor** (2026-07-04; **in-game GREEN PENDING**) — **right-click any grid cell** (AB/WSG/AV) to override its default (unmodified-click) callout in a native `StaticPopup`; blank reverts to the generated default. Overrides persist under `TitanBgGeneralSaved.customCallouts` keyed by `grid:cell:row` (shared `CalloutKey`/`Get`/`SetCalloutOverride`; `gridId` threaded through `BuildColGrid`). Modifier clicks (Shift/Ctrl/Alt) keep their generated messages; live suffixes (AB node state / WSG FC) still append so a custom base keeps enrichment. Tooltip shows the active custom text (gold) + a "right-click: customize" hint. **Verify next session:** right-click a cell, save custom text, left-click → confirm it sends (with any live suffix) and survives `/reload`; blank-save reverts.
